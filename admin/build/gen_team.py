...
<p><b>Each role also owns its own work plan</b>, as files: <code>issues/open/</code>, <code>issues/blocked/</code> and <code>issues/done/</code>, on the <a href="https://issues-fs.sgit.ai/lite/index.html">Issues-FS-lite</a> pattern where the folder <em>is</em> the status and a status change is a <code>git mv</code>. <a href="issues.html"><b>Browse the issue tree &rarr;</b></a> &middot; <a href="workflow.html"><b>the state map</b></a> &mdash; the states a change moves through, each with the door it must pass, and which doors are shut &middot; <a href="ISSUES.md">the pattern, and the writer rule</a></p>
