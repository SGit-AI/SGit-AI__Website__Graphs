...
| **The record** | `v2/briefs/` (26 founder memos since the freeze, verbatim), `admin/versions*.html` (every release, narrated) |
...
admin/          the build: generators, validator, tests, release history
  ...
  tests/        run.mjs (the runner) + <area>.test.mjs — 103 tests, six suites
...
```bash
python3 admin/build/gen_coregraph.py     # the core graph: every document folder, document -> word
...
python3 admin/build/gen_lexicon.py       # the lexicon scopes
python3 admin/build/gen_packs.py         # the concepts-and-evidence pack (html + pdf)
