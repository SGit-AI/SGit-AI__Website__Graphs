...
export const COMMANDS = [
  ...
  { name: 'show_subtree_only', level: 'view',
    description: 'Filter the graph to the selected node’s subtree only (or back to everything). Pass id to select first.',
    properties: { on: { type: 'boolean', description: 'true filters to the subtree, false shows everything.' },
      id: { type: 'string', description: 'Select this node first (optional).' } }, required: ['on'] },
  { name: 'toggle_doc_tree', level: 'view',
    description: 'Overlay the document’s section tree on the graph (true) or remove it (false), so anchors hang from their sections.',
