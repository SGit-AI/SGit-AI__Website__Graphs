...
The only artefact here that is **both a complete narrative and a machine-readable file**. One configuration fact — two admin accounts without 2FA — carried through the wrong acceptor, a governance air gap, a five-whys chain, and up to the board and the regulator.
...
::: warn
**The file is not mirrored here yet.** The JSON lives in the source repository, which is not public. Publishing it as a download from this site — with its ontology brief beside it — is [task T1 on the comms board](../admin/comms.html) and the single highest-value thing this site could add. Until it is here, this page describes it rather than pretending to serve it.
:::
...
- **“facts only in phase one”** — nothing hypothetical enters the graph
...
`Risk` (9) · `Actor` (9) · `Evidence` (6) · `Interval` (6) · `Fact` (5) · `Impact` (3) · `System` (2) · `Vulnerability` (2) · `DataClass` (2) · `Asset` · `ThreatAgent` · `Attack` (MITRE T1110.004) · `Obligation` · `Register` · `Twin` · `Agent`
