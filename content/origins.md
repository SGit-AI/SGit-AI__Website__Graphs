...
6. <span class="when">10 – 11 June · phase 5: the import, half-executed</span> <span class="what">An agent notices the assumption gap. Ten Issues-FS documents are imported and <code>library/concepts/</code> is created (phase 1 of the import memo). <b>Phase 2, adding the cross-reference so agents can find them, was specified and never executed.</b> That is why this book exists; see below.</span>
...
~ **The fix is an address.** Now that this book exists, every other publication and every future session can be pointed at one place, and the one-line edit to `CLAUDE.md` can finally be made. It is ask N1 on [the comms board](../admin/comms.html), the project's public list of asks and tasks.
:::

The generalisable version: **a document that nothing links to is, for a reader who follows links, a document that does not exist.** That is the same finding an agent reported when it tried to consume the parent project's documentation and could not follow the links, and it is why the companion site's [llms.txt](../llms.txt) (its index file for AI agents) carries facts rather than topic labels.
...
| What | What happened |
|---|---|
| **The Lexicon as a shipped package** | Proposed 5 February, imported 11 June, never referenced again. The anchor-node *concept* survived and is [in the grammar](../grammar/index.html#anchor-nodes); the package did not. |
...
| **MGraph-DB as the store** | Named repeatedly, deferred every time: *“for now let's keep it simple.”* File-based won, and [*What ships, what is argued*](../shipped/index.html) says so. |
...
::: agent
If you are working in this corpus: the philosophy lives in `library/concepts/`, it is referenced by four files, and it is **not** reachable from the entry-point file you were given. Read it directly rather than assuming that working forwards will surface it. Everything at [altitude 1](../start/index.html) of this book comes from those documents.
:::
